...
public class ProductionService {
    ...
    public void produceItems() {
        ...
        for (BuildingEntity building : buildings) {
            ...
            Optional<ItemEntity> existingItem = building.getItems().stream().filter(i -> i.getName().equals(itemName))
                    .findFirst();
            ...
            if (existingItem.isPresent()) {
                ...
                ItemEntity newItem = ItemEntity.builder().name(itemName).unit(ItemUnit.PIECE) // Default
                        .price(BigDecimal.TEN) // Default base price
                        .cost(BigDecimal.ZERO) // Default cost
                        .quantity(building.getProductionRate().intValue()).qualityScore(BigDecimal.ONE)
                        .tier(ItemTier.LOW).building(building).build();
    ...
    private String getItemNameForBuilding(BuildingEntity building) {
        if (building.getSubType() != null && building.getSubType().getProducedItemNames() != null) {
            ...
            /*
             * if (items.length > 0 && items[0] != null && !items[0].isEmpty()) { return
             * items[0]; // Return first produced item }
             */
        }
    ...
    private boolean consumeRawMaterials(BuildingEntity building, String productName) {
        ...
        Optional<ItemEntity> materialOpt = building.getItems().stream()
                .filter(i -> i.getName().equals(finalRequiredMaterial)).findFirst();
