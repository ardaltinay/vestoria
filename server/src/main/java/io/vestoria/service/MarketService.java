...
public class MarketService {
  ...
  public MarketEntity listItem(UUID userId, UUID itemId, ListItemRequestDto request) {
    ...
    Optional<MarketEntity> existingListing = marketRepository.findBySellerAndItemNameAndPriceAndIsActiveTrue(
        user, item.getName(), request.getPrice());
    ...
    messagingTemplate.convertAndSend("/topic/market", io.vestoria.dto.response.MarketUpdateDto.builder()
        .type("LIST")
        .id(savedItem.getId())
        .itemName(savedItem.getItem().getName())
        .quantity(savedItem.getQuantity())
        .price(savedItem.getPrice())
        .sellerName(savedItem.getSeller().getUsername())
        .build());
    ...
    while (attempt < maxRetries) {
      try {
        ...
        messagingTemplate.convertAndSend("/topic/market", io.vestoria.dto.response.MarketUpdateDto.builder()
            ...
            .quantity(quantity) // Amount bought
        ...
        if (attempt == maxRetries) {
          throw new BusinessRuleException("Yoğunluk nedeniyle işlem gerçekleştirilemedi. Lütfen tekrar deneyiniz.");
        }
  ...
  public List<MarketTrendDto> getMarketTrends() {
    ...
    List<String> itemNames = topItems.stream()
        .map(TrendingItemDto::getItemName)
    ...
    return topItems.stream().map(item -> {
      ...
      BigDecimal minPrice = (stats != null && stats.getMinPrice() != null) ? stats.getMinPrice() : BigDecimal.ZERO;
