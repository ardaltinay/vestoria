...
public class BuildingConverter {
    ...
    public BuildingProductionTypeDto toProductionTypeDto(BuildingSubType subType) {
        return BuildingProductionTypeDto.builder().value(subType.name()).label(subType.getLabel())
                .description(subType.getDescription()).parentType(subType.getParentType()).build();
    }
    ...
    public BuildingResponseDto toResponseDto(BuildingEntity entity) {
        ...
        return BuildingResponseDto.builder().id(entity.getId()).name(entity.getName()).label(label)
                .description(description).type(entity.getType()).tier(entity.getTier()).subType(entity.getSubType())
                .productionRate(entity.getProductionRate()).maxStock(entity.getMaxStock())
                .maxSlots(entity.getMaxSlots()).cost(entity.getCost()).status(entity.getStatus())
                .productionEndsAt(entity.getProductionEndsAt()).salesEndsAt(entity.getSalesEndsAt())
                .isProducing(entity.getIsProducing()).isSelling(entity.getIsSelling())
                .currentStock(entity.getItems() != null
                        ? entity.getItems().stream().mapToInt(ItemEntity::getQuantity).sum()
                        : 0)
                .items(entity.getItems() != null
                        ? entity.getItems().stream().map(itemConverter::toItemResponseDto).collect(Collectors.toList())
                        : Collections.emptyList())
                .lastRevenue(entity.getLastRevenue()).build();
    }
}
