<template>
  <div class="p-6">
    ...
    <div v-else class="space-y-6">
      ...
      <div v-if="!item.subType" class="bg-gradient-to-r from-cyan-500 to-blue-600 rounded-xl p-6 shadow-lg text-white">
        <h2 class="text-xl font-bold mb-2">Üretime Başla</h2>
        <p class="text-cyan-100 mb-4">Bu fabrika henüz üretim yapmıyor. Üretime başlamak için bir tür seçin:</p>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div v-for="option in productionOptions" :key="option.value" 
            ...
            <p class="text-sm text-cyan-100">{{ option.description }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
        ...
        <div class="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-4 sm:p-5 border border-purple-200">
          <div class="flex items-center justify-between">
            <div>
              <div class="text-xs sm:text-sm font-medium text-purple-600 mb-1">Tür</div>
              <div class="text-2xl sm:text-3xl font-bold text-purple-900">{{ item.subType || 'Seçilmedi' }}</div>
            </div>
...
async function fetchProductionTypes() {
  try {
    ...
    productionOptions.value = response.data.filter(t => t.parentType === 'FACTORY')
  } catch (error) {
    console.error('Failed to fetch production types:', error)
  }
}

const selectProduction = async (subType) => {
  try {
    await BuildingService.setProduction(item.value.id, subType)
    await store.load()
  } catch (error) {
