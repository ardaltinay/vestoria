<template>
  <div class="p-6">
    ...
    <div v-else class="space-y-6">
      ...
      <div v-if="!item.subType" class="bg-gradient-to-r from-amber-500 to-yellow-600 rounded-xl p-6 shadow-lg text-white">
        <h2 class="text-xl font-bold mb-2">Üretime Başla</h2>
        <p class="text-amber-100 mb-4">Bu çiftlik henüz üretim yapmıyor. Üretime başlamak için bir tür seçin:</p>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div v-for="option in productionOptions" :key="option.value" 
            ...
            <p class="text-sm text-amber-100">{{ option.description }}</p>
          </div>
        </div>
      </div>
...
const selectProduction = async (subType) => {
  ...
  try {
    ...
    await store.load()
    addToast('Üretim türü başarıyla seçildi!', 'success')
  } catch (error) {
...
onMounted(async () => {
  ...
  try {
    ...
    productionOptions.value = types.data.filter(t => t.parentType === 'FARM')
  } catch (e) {
