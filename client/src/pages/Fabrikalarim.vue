<template>
  <div class="min-h-screen bg-[#f8fafc] font-sans selection:bg-blue-100 selection:text-blue-900 pb-20">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
          ...
          <button 
            ...
            class="w-full sm:w-auto group flex items-center justify-center gap-2 px-4 sm:px-6 py-2.5 sm:py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition-all shadow-lg shadow-blue-600/20 active:scale-95 touch-target"
          >
            <div class="w-5 h-5 sm:w-6 sm:h-6 rounded-lg bg-blue-500 flex items-center justify-center group-hover:bg-blue-400 transition-colors">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <!-- Empty State -->
      <div v-else-if="factories.length === 0" class="bg-white rounded-2xl sm:rounded-3xl border border-dashed border-slate-300 p-8 sm:p-16 text-center max-w-2xl mx-auto mt-6 sm:mt-10">
        <div class="w-16 h-16 sm:w-24 sm:h-24 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6">
          <span class="text-2xl sm:text-4xl">🏭</span>
        </div>
        <h3 class="text-xl sm:text-2xl font-black text-slate-800 mb-2 sm:mb-3">Henüz Fabrikanız Yok</h3>
        <p class="text-sm sm:text-lg text-slate-500 mb-6 sm:mb-8 leading-relaxed">
          Sanayi devrimine katılmak için ilk fabrikanızı kurun.
        </p>
        <button 
          @click="showWizard = true"
          class="w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-4 bg-blue-600 text-white rounded-xl sm:rounded-2xl font-bold text-base sm:text-lg hover:bg-blue-700 transition-all shadow-xl shadow-blue-600/20 hover:-translate-y-1 touch-target"
        >
          Hemen Başla
        </button>
      </div>
