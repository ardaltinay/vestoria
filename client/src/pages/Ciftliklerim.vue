<template>
  <div class="min-h-screen bg-[#f8fafc] font-sans selection:bg-emerald-100 selection:text-emerald-900 pb-20">
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <button 
            ...
            class="group flex items-center justify-center gap-2 px-6 py-3 bg-emerald-600 text-white rounded-xl font-bold hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-600/20 active:scale-95"
          >
            <div class="w-6 h-6 rounded-lg bg-emerald-500 flex items-center justify-center group-hover:bg-emerald-400 transition-colors">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <!-- Empty State -->
      <div v-else-if="farms.length === 0" class="bg-white rounded-3xl border border-dashed border-slate-300 p-16 text-center max-w-2xl mx-auto mt-10">
        <div class="w-24 h-24 bg-emerald-50 rounded-full flex items-center justify-center mx-auto mb-6">
          <span class="text-4xl">🌾</span>
        </div>
        <h3 class="text-2xl font-black text-slate-800 mb-3">Henüz Çiftliğiniz Yok</h3>
        <p class="text-slate-500 text-lg mb-8 leading-relaxed">
          Üretime başlamak için ilk çiftliğinizi kurun. Çiftlikler size düzenli gelir ve hammadde sağlar.
        </p>
        <button 
          @click="showWizard = true"
          class="px-8 py-4 bg-emerald-600 text-white rounded-2xl font-bold text-lg hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-600/20 hover:-translate-y-1"
        >
          Hemen Başla
        </button>
      </div>
