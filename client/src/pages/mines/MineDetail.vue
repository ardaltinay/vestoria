<template>
  <div class="min-h-screen bg-stone-50 font-sans selection:bg-amber-200 selection:text-amber-900">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        <!-- Header Section -->
        <div class="relative z-10 mb-8 bg-white/60 backdrop-blur-md rounded-3xl p-6 border border-white/50 shadow-xl shadow-stone-900/5">
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div class="flex items-center gap-5">
              <div class="relative group">
                ...
                <!-- Level Badge -->
                <div v-if="item" class="absolute -bottom-2 -right-2 bg-amber-500 text-white text-xs font-black px-2 py-1 rounded-lg shadow-md border-2 border-white">
                  LVL {{ item.tier === 'SMALL' ? 1 : item?.tier === 'MEDIUM' ? 2 : 3 }}
                </div>
              </div>
              
              <div>
                <h1 class="text-3xl sm:text-4xl font-black text-stone-800 tracking-tight mb-1 drop-shadow-sm">{{ item?.name || 'Maden' }}</h1>
        ...
        <div v-if="!item" class="text-center py-20">
          <div class="inline-block animate-spin rounded-full h-12 w-12 border-4 border-stone-300 border-t-stone-600 mb-4"></div>
          <div class="text-stone-400 text-lg font-medium">Maden verileri yükleniyor...</div>
        </div>
        ...
        <div v-else class="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8">
          ...
          <div class="lg:col-span-8 space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div class="bg-white/70 backdrop-blur-sm rounded-2xl p-5 border border-white/60 shadow-lg shadow-stone-200/50 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
                <div class="flex items-start justify-between mb-3">
                  <div class="p-2 bg-stone-100/50 rounded-xl text-stone-600 group-hover:bg-stone-500 group-hover:text-white transition-colors duration-300">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="h-2 w-full bg-stone-100 rounded-full overflow-hidden border border-stone-100">
                  <div class="h-full bg-gradient-to-r from-amber-400 to-amber-600 rounded-full transition-all duration-700 ease-out shadow-[0_0_10px_rgba(245,158,11,0.5)]" :style="{ width: `${Math.min((currentStock / item.maxStock) * 100, 100)}%` }"></div>
                </div>
              </div>
              ...
              <div class="p-6 sm:p-8">
                <div class="flex items-center justify-between mb-8">
                  ...
                  <div class="flex items-center gap-2 px-3 py-1.5 rounded-full border" 
                    ...
                    <span class="text-xs font-bold uppercase tracking-wide">{{ isProducing ? 'KAZI SÜRÜYOR' : 'DURDURULDU' }}</span>
                  </div>
                </div>
                ...
                <div v-if="isProducing" class="flex flex-col items-center justify-center py-6">
                  ...
                  <div v-if="!canCollect" class="text-center w-full max-w-md">
                     <div class="bg-stone-50 rounded-xl p-4 border border-stone-200 flex items-center justify-center gap-3">
                       ...
                       <span class="text-stone-600 font-medium">Kazı işlemi devam ediyor, lütfen bekleyin.</span>
                     </div>
                  </div>
                  ...
                  >
                    ...
                    <span class="relative z-10">MADENLERİ TOPLA</span>
                  </button>
                </div>
                ...
                <div v-else class="text-center py-8">
                  ...
                  <h4 class="text-lg font-bold text-stone-700 mb-2">Maden Beklemede</h4>
                  <p class="text-stone-500 mb-8 max-w-md mx-auto">Maden şu anda aktif değil. Yeni bir kazı emri vermek için aşağıdaki butonu kullanın.</p>
                  ...
                  >
                    ...
                    <span class="relative z-10">KAZIYI BAŞLAT</span>
                  </button>
                </div>
              </div>
            </div>
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-stone-200 overflow-hidden">
              ...
              <div class="p-0">
                <div v-if="!item.items || item.items.length === 0" class="p-12 text-center">
                  ...
                  <p class="text-stone-400 font-medium">Depo boş. Kazı yaparak stok oluşturun.</p>
                </div>
          ...
          <div class="lg:col-span-4 space-y-6">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-stone-200 p-6 relative overflow-hidden">
              ...
              <h3 class="text-sm font-bold text-stone-400 uppercase tracking-wider mb-4 relative z-10">Maden Bilgileri</h3>
              ...
              <div class="space-y-4 relative z-10">
                ...
                >
                  <div class="flex items-center gap-3">
                    ...
                    <div class="text-left">
                      <div class="text-sm font-bold text-stone-800 group-hover:text-amber-700">Seviye Yükselt</div>
                      <div class="text-xs text-stone-400 font-medium">Kapasite Artışı</div>
                    </div>
                  </div>
                  <div class="text-xs font-black bg-stone-100 px-2 py-1 rounded-lg text-stone-500 group-hover:bg-amber-100 group-hover:text-amber-600 transition-colors">
                ...
                >
                  <div class="flex items-center gap-3">
                    ...
                    <div class="text-left">
                      ...
                      <div class="text-xs text-stone-400 font-medium">Madeni Kapat</div>
                    </div>
                  </div>
                </button>
              </div>
            ...
            <div class="bg-gradient-to-br from-amber-50 to-orange-100 rounded-3xl p-6 text-amber-900 shadow-lg shadow-amber-900/5 relative overflow-hidden">
              ...
              <p class="text-sm text-amber-800 leading-relaxed relative z-10">
                Madenler değerli kaynaklar sağlar. Üretim süreleri uzun olabilir, planlamanı iyi yap!
              </p>
            </div>
          </div>
        </div>
      ...
      <Teleport to="body">
        ...
        >
          <div class="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl relative border border-stone-100">
            <div class="p-6 border-b border-stone-100 flex justify-between items-center bg-stone-50/50">
              <h2 class="text-xl font-black text-stone-800">Kazıyı Başlat</h2>
            ...
            <div class="p-6 space-y-6">
              <SelectBox
                ...
                label="Çıkarılacak Maden"
                placeholder="Bir maden seçin..."
                value-key="id"
                label-key="name"
              />
                ...
                <div class="flex gap-3">
                  ...
                  <div>
                    ...
                    <p class="text-amber-700 text-sm mt-0.5">
                      Bu kazı işlemi yaklaşık <strong class="text-amber-900">{{ buildingConfig?.productionDuration || '-' }} dakika</strong> sürecektir.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div class="flex gap-3 pt-2">
                ...
                <button 
                  ...
                  class="flex-1 py-4 bg-amber-500 text-white font-bold rounded-2xl hover:bg-amber-600 transition-all shadow-lg shadow-amber-500/20 disabled:opacity-50 disabled:cursor-not-allowed transform active:scale-95"
                >
                  Başlat
                </button>
              </div>
            ...
            <div class="p-6 space-y-6">
              ...
              <div class="flex gap-3 pt-2">
                ...
                <button 
                  ...
                  class="flex-1 py-4 bg-amber-500 text-white font-bold rounded-2xl hover:bg-amber-600 transition-all shadow-lg shadow-amber-500/20 transform active:scale-95"
                >
                  Onayla ve Yükselt
                </button>
              </div>
...
const buildingConfig = computed(() => item.value ? configStore.getConfig('MINE', item.value.tier) : null)
...
const confirmClose = async () => {
  ...
  try {
    ...
    await authStore.fetchUser()
    await store.load()
    window.location.href = '/home/mines'
  } catch (error) {
    console.error(error)
...
onMounted(async () => {
  ...
  productionOptions.value = gameDataStore.getItemsByType('MINE')
